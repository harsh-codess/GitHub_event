...
- Document each reusable component `Props` interface with a short TSDoc/JSDoc block so the component contract is clear at a glance.
...
- Comments in `.astro` files should explain intent or non-obvious decisions; do not add comments that only restate markup or code mechanics.
